...
void    ScalarConverter::is_float(String &to_conv)
{
    ...
    try
    {
        ...
        if (test == -HUGE_VALF || test == HUGE_VALF)
            ...
            in_float = std::strtod(to_conv.c_str(), NULL);

        if (in_float >= 32 && in_float <= 126 && ScalarConverter::is_double_integer(in_float))
        ...
        if (ScalarConverter::is_double_integer(in_float))
        {    
            std::cout << "float: " << static_cast<double>(in_float) << ".0f" << std::endl;
            std::cout << "double: " << in_float << ".0" << std::endl;
        }
        else
        {
            std::cout << "float: " << static_cast<double>(in_float) << "f" << std::endl;
            std::cout << "double: " << in_float << std::endl;
        }
    }
...
void    ScalarConverter::is_double(String &to_conv)
{
    ...
    try
    {
        ...
        if (in_double <= INT_MAX && in_double >= INT_MIN)
            std::cout << static_cast<int>(in_double) << std::endl;
        else
            std::cout << "int: impossible" << std::endl;
        ...
        {    
            if (ScalarConverter::is_double_integer(in_double))
                std::cout << static_cast<float>(in_double) << ".0f" << std::endl;
            else
                std::cout << static_cast<float>(in_double) << "f" << std::endl;
        }
        else 
            std::cout << "float: impossible" << std::endl;
